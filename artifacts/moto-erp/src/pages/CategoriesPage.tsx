...
export default function CategoriesPage() {
  ...
  const { data: allSubcategories } = useQuery({
    ...
    queryFn: () => apiFetch<SubcategoryRecord[]>("/subcategories"),
  });
  ...
  const createMutation = useMutation({
    mutationFn: (values: CategoryFormValues) =>
      apiFetch("/categories", { method: "POST", body: JSON.stringify(values) }),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, values }: { id: number; values: CategoryFormValues }) =>
      apiFetch(`/categories/${id}`, { method: "PUT", body: JSON.stringify(values) }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/categories"] });
  ...
  const openAdd = () => {
    form.reset({ name: "", description: "", imageUrl: null });
    setIsAddOpen(true);
  };

  const startEdit = (cat: CategoryRecord) => {
    form.reset({ name: cat.name, description: cat.description ?? "", imageUrl: cat.imageUrl ?? null });
                ...
                : categories?.map((cat) => {
                  ...
                  return (
                    <React.Fragment key={cat.id}>
                      ...
                      {isExpanded && subs.length > 0 && subs.map(sub => (
                        <TableRow key={`sub-${sub.id}`} className="bg-orange-50/50 dark:bg-orange-950/10">
                          <TableCell></TableCell>
                          <TableCell>
                            {sub.imageUrl
                              ? <img src={sub.imageUrl} alt={sub.name} className="h-8 w-8 rounded object-cover border" />
                              : null
                            }
                          </TableCell>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder={t("categories.categoryName")} {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
