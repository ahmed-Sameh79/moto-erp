...
export default function SubCategoriesPage() {
  ...
  const { data: subcategories, isLoading } = useQuery({
    ...
    queryFn: () => {
      const url = filterCategoryId !== "all"
        ? `/subcategories?categoryId=${filterCategoryId}`
        : "/subcategories";
      return apiFetch<SubcategoryRecord[]>(url);
    },
  });
  ...
  const createMutation = useMutation({
    mutationFn: (values: SubcategoryFormValues) =>
      apiFetch("/subcategories", { method: "POST", body: JSON.stringify(values) }),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, values }: { id: number; values: SubcategoryFormValues }) =>
      apiFetch(`/subcategories/${id}`, { method: "PUT", body: JSON.stringify(values) }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/subcategories"] });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => apiFetch(`/subcategories/${id}`, { method: "DELETE" }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/subcategories"] });
  ...
  const openAdd = () => {
    form.reset({ name: "", description: "", categoryId: undefined, imageUrl: null });
    setIsAddOpen(true);
  };

  const startEdit = (sub: SubcategoryRecord) => {
    form.reset({ name: sub.name, description: sub.description ?? "", categoryId: sub.categoryId, imageUrl: sub.imageUrl ?? null });
    setEditingSub(sub);
  };

  const onSubmit = (values: SubcategoryFormValues) => {
    if (editingSub) {
      updateMutation.mutate({ id: editingSub.id, values });
    } else {
    ...
    <div className="p-6 space-y-6">
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            if (confirm(t("subcategories.confirmDelete"))) {
      ...
      <Dialog open={isAddOpen || !!editingSub} onOpenChange={(v) => { if (!v) closeDialog(); }}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{editingSub ? t("subcategories.editSubcategory") : t("subcategories.addSubcategory")}</DialogTitle>
          </DialogHeader>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t("subcategories.category")}</FormLabel>
                    <Select
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t("subcategories.subcategoryName")}</FormLabel>
                    <FormControl>
                      <Input placeholder={t("subcategories.subcategoryName")} {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea placeholder={t("common.description")} rows={3} {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
