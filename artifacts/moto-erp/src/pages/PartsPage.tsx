...
const partSchema = z.object({
  ...
  binId: z.number().optional().nullable(),
...
export default function PartsPage() {
  ...
  const { data: parts, isLoading } = useQuery({
    ...
    queryFn: () => apiFetch<PartRecord[]>(`/parts?search=${search}`),
  });

  const { data: warehouses } = useQuery({
    queryKey: ["/warehouses"],
    queryFn: () => apiFetch<WarehouseOption[]>("/warehouses"),
  });

  const { data: bins } = useQuery({
    queryKey: ["/bins"],
    queryFn: () => apiFetch<BinOption[]>("/bins"),
  });
  ...
  const { data: subcategories } = useQuery({
    ...
    queryFn: () => apiFetch<SubcategoryOption[]>("/subcategories"),
  });
  ...
  const filteredBins = bins?.filter(b => b.warehouseId === selectedWarehouseId) ?? [];
  const filteredSubcategories = subcategories?.filter(s => s.categoryId === selectedCategoryId) ?? [];

  useEffect(() => {
    form.setValue("binId", null);
  }, [selectedWarehouseId]);
  ...
  const createMutation = useMutation({
    mutationFn: (values: PartFormValues) => apiFetch("/parts", {
      method: "POST",
      body: JSON.stringify({
        ...values,
  ...
  const updateMutation = useMutation({
    mutationFn: (values: PartFormValues) => apiFetch(`/parts/${editingPart!.id}`, {
      method: "PUT",
      body: JSON.stringify({
        ...values,
        costPrice: values.costPrice.toString(),
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => apiFetch(`/parts/${id}`, { method: "DELETE" }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/parts"] });
  ...
  const startEdit = (part: PartRecord) => {
    setEditingPart(part);
    const sub = subcategories?.find(s => s.id === part.subcategoryId);
    form.reset({
      sku: part.sku,
      name: part.name,
      ...
      subcategoryId: part.subcategoryId ?? null,
      quantityOnHand: part.quantityOnHand,
      reorderPoint: part.reorderPoint,
      costPrice: parseFloat(part.costPrice),
      sellingPrice: parseFloat(part.sellingPrice),
      warehouseId: part.warehouseId ?? null,
      binId: part.binId ?? null,
  ...
  const getSubcategoryName = (part: PartRecord) => {
    if (part.subcategoryName) {
      return part.categoryName ? `${part.categoryName} › ${part.subcategoryName}` : part.subcategoryName;
    }
    if (!part.subcategoryId) return null;
    const sub = subcategories?.find(s => s.id === part.subcategoryId);
    if (!sub) return null;
      ...
      <div className="flex justify-between items-center">
        ...
        <Dialog open={isAddOpen || !!editingPart} onOpenChange={(open) => { if (!open) closeDialog(); }}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editingPart ? t("parts.editPart") : t("parts.addPart")}</DialogTitle>
            </DialogHeader>
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t("parts.sku")}</FormLabel>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t("parts.partName")}</FormLabel>
                        <FormControl><Input {...field} /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                ...
                <div className="grid grid-cols-3 gap-4">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t("parts.condition")}</FormLabel>
                        <Select onValueChange={field.onChange} value={field.value}>
                          <FormControl>
                            <SelectTrigger>
                              <SelectValue placeholder={t("parts.condition")} />
                            </SelectTrigger>
                          </FormControl>
                          <SelectContent>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t("parts.quantityOnHand")}</FormLabel>
                        <FormControl>
                          <Input
                            type="number"
                            {...field}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t("parts.reorderPoint")}</FormLabel>
                        <FormControl>
                          <Input
                            type="number"
                            {...field}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t("parts.costPrice")}</FormLabel>
                        <FormControl>
                          <Input
                            type="number"
                            step="0.01"
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t("parts.sellingPrice")}</FormLabel>
                        <FormControl>
                          <Input
                            type="number"
                            step="0.01"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t("parts.warehouse")}</FormLabel>
                        <Select
                          onValueChange={v => field.onChange(v === "none" ? null : parseInt(v))}
                          value={field.value != null ? String(field.value) : "none"}
                        >
                          <FormControl>
                            <SelectTrigger>
                              <SelectValue placeholder={t("parts.warehouse")} />
                            </SelectTrigger>
                          </FormControl>
                          <SelectContent>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t("parts.bin")}</FormLabel>
                        <Select
                          onValueChange={v => field.onChange(v === "none" ? null : parseInt(v))}
                          value={field.value != null ? String(field.value) : "none"}
                        ...
                        >
                          <FormControl>
                            <SelectTrigger>
                              <SelectValue placeholder={selectedWarehouseId ? t("parts.selectBin") : t("parts.selectWarehouseFirst")} />
                            </SelectTrigger>
                          </FormControl>
                          <SelectContent>
            ...
            {isLoading ? (
              ...
              parts?.map((part) => (
                <TableRow key={part.id}>
                  ...
                  <TableCell>
                    {getSubcategoryName(part)
                      ? (
                        <Badge variant="outline" className="text-orange-600 border-orange-200 bg-orange-50 dark:bg-orange-950/20 text-xs font-normal">
                          {getSubcategoryName(part)}
                        </Badge>
                      )
                  ...
                  <TableCell className="text-right">
                    <span className={part.quantityOnHand <= part.reorderPoint ? "text-red-600 font-bold" : ""}>
                      {part.quantityOnHand}
                    </span>
                  </TableCell>
                  ...
                  <TableCell>
                    {part.binLabel ? (
                      <Badge variant="outline" className="font-mono text-xs">{part.binLabel}</Badge>
                    ) : "-"}
                  </TableCell>
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                      <Button variant="ghost" size="icon" onClick={() => startEdit(part)}>
                      ...
                      <Button 
                        ...
                        onClick={() => {
                          if (confirm(t("parts.confirmDelete"))) {
