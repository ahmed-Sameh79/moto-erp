...
function BinTree({ bins }: { bins: BinRecord[] }) {
  ...
  return (
    <div className="space-y-1">
      <p className="text-xs text-muted-foreground mb-3">{totalBins} bin{totalBins !== 1 ? "s" : ""} across {zones.length} zone{zones.length !== 1 ? "s" : ""}</p>
      {zones.map(zone => {
        ...
        return (
          <div key={zone} className="border rounded-md overflow-hidden">
            <button
              type="button"
              className="w-full flex items-center justify-between px-3 py-2 bg-orange-50 hover:bg-orange-100 transition-colors text-sm font-semibold"
                ...
                {aisles.map(aisle => {
                  ...
                  return (
                    <div key={aisle} className="border-l-2 border-orange-200 pl-2">
                      <button
                        type="button"
                        className="w-full flex items-center justify-between px-2 py-1.5 hover:bg-gray-50 rounded text-sm"
                        onClick={() => toggleAisle(aisleKey)}
                      >
                        <span className="flex items-center gap-2 text-muted-foreground">
                          {aisleOpen ? <ChevronDown className="h-3 w-3" /> : <ChevronRight className="h-3 w-3" />}
                          Aisle {aisle}
                        </span>
                            ...
                            return (
                              <div key={shelf} className="border-l-2 border-gray-200 pl-2">
                                ...
                                <div className="flex flex-wrap gap-1 px-2 pb-2">
                                  {binsInShelf.map(b => (
                                    <Badge key={b.id} variant="outline" className="font-mono text-xs cursor-default" title={`Bin ID: ${b.id}`}>
                                      {b.label}
                                    </Badge>
                                  ))}
                                </div>
...
export default function WarehousesPage() {
  ...
  const { data: bins, isLoading: isBinsLoading } = useQuery({
    queryKey: ["/bins", selectedWarehouse?.id],
    queryFn: () => apiFetch<BinRecord[]>(`/warehouses/${selectedWarehouse!.id}/bins`),
  ...
  const createWarehouseMutation = useMutation({
    mutationFn: (values: z.infer<typeof warehouseSchema>) =>
      apiFetch("/warehouses", { method: "POST", body: JSON.stringify(values) }),
    onSuccess: () => {
  ...
  const createBinMutation = useMutation({
    mutationFn: (values: z.infer<typeof binSchema>) =>
      apiFetch(`/warehouses/${values.warehouseId}/bins`, { method: "POST", body: JSON.stringify(values) }),
    onSuccess: () => {
          ...
          <Dialog open={isAddBinOpen} onOpenChange={setIsAddBinOpen}>
            <DialogTrigger asChild>
              <Button variant="outline">
                <Plus className="h-4 w-4 mr-2" /> {t("warehouses.addBin")}
              </Button>
            </DialogTrigger>
            <DialogContent>
          ...
          <Dialog open={isAddWarehouseOpen} onOpenChange={setIsAddWarehouseOpen}>
            <DialogTrigger asChild>
              <Button className="bg-orange-500 hover:bg-orange-600">
                <Plus className="h-4 w-4 mr-2" /> {t("warehouses.addWarehouse")}
              </Button>
            </DialogTrigger>
            <DialogContent>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {isLoading
          ...
          : warehouses?.map(w => (
            <Card
              key={w.id}
              className={`cursor-pointer transition-all ${selectedWarehouse?.id === w.id ? "ring-2 ring-orange-500 shadow-md" : "hover:shadow-sm"}`}
            ...
            >
              <CardHeader className="pb-2">
                <div className="flex justify-between items-start">
                  ...
                  <Badge variant="outline">{w.binCount ?? 0} Bins</Badge>
                </div>
